  ...
  exec(execCommand.toString("utf-8"), async (error, stdout, stderr) => {
    ...
    async function initSubBot() {
      ...
      const config = {
        ...
        browser: isCode ? ["Ubuntu", "Chrome", "110.0.5585.95"] : ["${namebot} (Sub Bot)", "Chrome", "2.0.0"],
        defaultQueryTimeoutMs: undefined,
        getMessage: async msgId => {
          if (store) {}
          return {
            conversation: "${namebot}Bot-MD"
          };
        }
      };
      ...
      async function handleConnectionUpdate(update) {
          ...
          qrMessage = await user.sendMessage(msg.chat, {
            ...
            contextInfo: {
              ...
              forwardedNewsletterMessageInfo: {
                newsletterJid: '120363411154070926@newsletter', 
                newsletterName: 'Sonic', 
                serverMessageId: -1
              }
            }
          }, { quoted: msg });
        ...
        if (connection === "close") {
          ...
          if (disconnectCode === DisconnectReason.restartRequired) {
            ...
            return await msg.reply("La conexion se ha cerrado, debera de conectarse manualmente usando el comando *.serbot* y reescanear el nuevo *QR.* Que fue enviada la primera vez que se hizo *SubBot*");
          } else if (disconnectCode === DisconnectReason.timedOut) {
            await closeConnection(false);
            return console.log("\nTiempo de conexion agotado, reconectando....");
          } else {
            console.log("\nRazon de la desconexion desconocida: " + (disconnectCode || "") + " >> " + (connection || ""));
          }
        }
        ...
        if (connection == "open") {
          ...
          await user.sendMessage(msg.chat, {
            text: args[0] ? "Esta¡ conectado(a)!! Por favor espere se esta¡ cargando los mensajes..." : "Conexion con exito al WhatsApp"
          }, { quoted: msg });
          if (!args[0]) {
