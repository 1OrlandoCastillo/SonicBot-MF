...
var handler = async (m, { conn, text }) => {
  ...
  // Inicializamos arreglo para factos usados globalmente si no existe
  if (!global.factosUsados) global.factosUsados = []

  // Si ya usamos todos los factos, reiniciamos la lista usada
  if (global.factosUsados.length >= global.factos.length) global.factosUsados = []

  // Elegimos factos que no se hayan usado
  let disponibles = global.factos.filter(f => !global.factosUsados.includes(f))

  // Elegimos uno al azar de los disponibles
  let elegido = disponibles[Math.floor(Math.random() * disponibles.length)]

  // Guardamos el elegido para evitar repetición
