...
const { writeExifImg } = require('../libs/functions');
...
const handler = async (msg, { conn, args }) => {
  try {
    ...
    if (quoted && ctx?.participant) {
      ...
      textQuoted = extractQuotedText(quoted);
    ...
    if (colors[firstWord]) {
      ...
      content = afterColor.length > 0 ? afterColor : (textQuoted || ' ');
    } else {
    ...
    if (!data?.result?.image) throw new Error('No se generó la imagen de quote');
